...
def _render_alert_settings(patient: dict, username: str):
    """Settings — update family alert emails and test the alert system."""
    st.markdown("### ⚙️ Settings & Alerts")

    current_emails = patient.get("family_emails", [])
    st.markdown("#### 📧 Family Alert Emails")
    st.markdown(
        "Enter the email addresses of family members or carers who should "
        "receive automatic alerts. They will get an email every time the patient "
        "submits their daily check-in, and an immediate alert if blood pressure "
        "is high or critical."
    )

    if current_emails:
        st.success(
            f"✅ **{len(current_emails)} address(es) registered:** " +
            "  |  ".join(current_emails)
        )
    else:
        st.warning("⚠️ No alert emails added yet. Add them below.")
    ...
    if st.button("💾 Save Email Addresses"):
        parsed = [e.strip().lower() for e in new_raw.splitlines()
                  if e.strip() and "@" in e]
        if not parsed:
            st.error("Please enter at least one valid email address.")
        else:
            update_patient_field(username, "family_emails", parsed)
            st.session_state["patient"]["family_emails"] = parsed
            st.success(f"✅ Saved {len(parsed)} address(es): " + ", ".join(parsed))
            import time; time.sleep(1); st.rerun()

    st.markdown("---")
    st.markdown("#### 🧪 Send a Test Alert")
    st.markdown(
        "Tap the button to send a test email to all registered addresses. "
        "Check the inbox (and spam folder) to confirm it arrived."
    )
    if st.button("📤 Send Test Alert Now"):
        emails = patient.get("family_emails", [])
        if not emails:
            st.error("Add at least one email address above first.")
        else:
            ...
            sent = send_alert_email(
                ...
                alert_type="daily_complete",
                message=(
                    f"This is a TEST alert from the Stroke Recovery Monitor "
                    f"for {patient['patient_name']}. "
                    "If you received this, alerts are working correctly."
                ),
                details={
                    "Test Status": "✅ Alert system working",
            ...
            if sent:
                st.success(
                    f"✅ Test alert delivered to: {', '.join(emails)}\n\n"
                    "Check those inboxes now — also check spam/junk. "
                    "If it arrived, real alerts will work automatically."
                )
            else:
                st.error(
                    "❌ Test failed. The app owner (Samuel) needs to add "
                    "**RESEND_API_KEY** to Streamlit Secrets. "
    ...
    if api_key:
        st.success("✅ Alert system is configured and ready")
    else:
        st.error("❌ Alert system not configured — RESEND_API_KEY missing from Streamlit Secrets")

    emails_now = patient.get("family_emails", [])
    if emails_now:
        st.success(f"✅ {len(emails_now)} alert address(es) registered")
    else:
        st.warning("⚠️ No alert addresses — add them above")
