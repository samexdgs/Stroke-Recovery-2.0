...
def _render_exercise_guide():
    ...
    st.markdown("*These exercises are designed for stroke survivors. Always stop if pain is above 7.*")

    for ex in EXERCISES:
        diff_color = {"Beginner": "#059669", "Intermediate": "#d97706",
                      "All levels": "#4f46e5"}.get(ex["difficulty"], "#6b7280")
        st.markdown(f"""
        <div class="ex-card">
            <div class="ex-title">{ex['icon']} {ex['name']}</div>
            <div class="ex-meta">
                <span style="color:{diff_color};font-weight:600;">{ex['difficulty']}</span> ·
                {ex['reps']} · {ex['duration']} · <em>Target: {ex['target']}</em>
            </div>
    ...
    Stop immediately if you feel chest pain, dizziness, or severe pain.
    </div>""", unsafe_allow_html=True)
