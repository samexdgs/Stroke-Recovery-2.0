...
from ml_engine import (
    load_models, predict, get_recommendations, get_exercise_lottie,
...
def page_login():
    ...
    with col_center:
        ...
        mode = st.radio("I am:", ["Patient", "Family Member (monitoring dashboard)"],
                        horizontal=True)

        st.markdown("#### Sign In")
        username = st.text_input("Patient username", placeholder="e.g. john_obi").strip().lower()
...
def page_register():
    ...
    st.info("This is done once — by the patient or their family coordinator — to create the profile.")

    r_name     = st.text_input("Patient's full name", key="r_name")
    r_username = st.text_input("Choose a username (no spaces)", key="r_username",
                               placeholder="e.g. john_obi or grace_peter").strip().lower()
...
def page_patient_dashboard(model_results, best_name):
    ...
    with st.sidebar:
        ...
        nav = st.radio("", ["📋 Daily Check-In", "📈 My Progress", "🏃 Exercise Guide", "📊 Model Info", "⚙️ Settings & Alerts"],
                       label_visibility="collapsed")
        st.markdown("---")
        # Show registered emails in sidebar as diagnostic
        emails_stored = patient.get("family_emails", [])
        if emails_stored:
            st.markdown("**📧 Alert emails registered:**")
            for em in emails_stored:
                st.markdown(f"• {em}")
        else:
            st.warning("⚠️ No alert emails registered. Add them in **Settings & Alerts**.")
        st.markdown("---")
        if st.button("🔓 Sign Out"):
            logout()

    st.markdown(f'<div class="app-title">🧠 Recovery Dashboard</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="app-sub">Welcome back, {name} · Day {days_post} of your recovery journey</div>',
                unsafe_allow_html=True)
    ...
    elif nav == "⚙️ Settings & Alerts":
        _render_alert_settings(patient, username)
...
def _render_checkin_form(patient, username, model_results, best_name, days_post):
    ...
    if submitted:
        ...
        # ── Trigger and send ALL alerts BEFORE st.rerun() ──────────────────
        # st.rerun() kills any in-progress SMTP connection, so every email
        # must complete before we call rerun. We collect results and show
        # them in the UI so the patient/carer can see what was sent.

        name         = patient["patient_name"]
        emails       = patient.get("family_emails", [])
        alert_log    = []   # collect what was sent for UI display

        # 1. BP alert (highest priority — send first)
        ...
        if is_bp_alert:
            ...
            save_alert(username, alert_type, bp_msg,
            ...
            alert_log.append(("BP Alert", bp_severity.upper(), sent))

        # 2. Recovery needs-attention alert (save to DB, no separate email —
        #    the daily completion email below will carry the status)
        if pred == 0:
            save_alert(
                username, "needs_attention",
                f"{name}'s recovery needs clinical attention today."
            )

        # 3. Daily completion alert — always sent, carries full summary
        completion_details = build_daily_alert_details(data, pred, proba)
        sent = send_alert_email(
            patient_name=name,
            family_emails=emails,
            alert_type="daily_complete",
            message=f"{name} has completed their daily recovery check-in.",
            details=completion_details
        )
        alert_log.append(("Daily Summary", "INFO", sent))

        # ── Show what happened with emails BEFORE rerun ─────────────────────
        if emails:
            for alert_name, severity, was_sent in alert_log:
                if was_sent:
                    st.success(
                        f"✅ **{alert_name}** alert sent to "
                        f"{len(emails)} contact(s): {', '.join(emails)}"
                    )
                else:
                    st.error(
                        f"❌ **{alert_name}** alert FAILED to send. "
                        "Check your SENDER_EMAIL and SENDER_PASSWORD in "
                        "Streamlit Settings → Secrets."
                    )
        else:
            st.info(
                "ℹ️ No family email addresses are registered for this patient. "
                "To enable alerts, re-register and add email addresses."
            )

        import time
        time.sleep(2)   # Give the user 2 seconds to read the alert status
        st.rerun()
...
def _exercise_card_html(ex: dict, diff_color: str) -> str:
    """
    Renders a complete exercise card as a self-contained HTML block.
    Uses the uploaded Lottie JSON loaded as base64 — no CDN, no network dependency.
    The bodymovin/lottie-web library is loaded from cdnjs (reliable CDN).
    Falls back to an animated SVG figure if the Lottie data is unavailable.
    """
    lottie_key   = ex.get("lottie_key", "")
    lottie_b64   = get_exercise_lottie(lottie_key)
    icon         = ex.get("icon", "🏃")
    ex_name      = ex["name"]
    reps         = ex["reps"]
    ...
    safe_id      = ex_name.replace(" ", "_").replace("&", "")

    if lottie_b64:
        animation_block = f"""
        <script src="https://cdnjs.cloudflare.com/ajax/libs/bodymovin/5.12.2/lottie.min.js"></script>
        <div style="background:#f0f4ff;border-radius:14px;
                    display:flex;align-items:center;justify-content:center;
                    height:240px;overflow:hidden;">
            <div id="anim_{safe_id}" style="width:220px;height:220px;"></div>
        </div>
        <script>
        (function(){{
            var data = JSON.parse(atob('{lottie_b64}'));
            lottie.loadAnimation({{
                container: document.getElementById('anim_{safe_id}'),
                renderer: 'svg',
                loop: true,
                autoplay: true,
                animationData: data
        ...
        <div style="display:flex;flex-direction:column;align-items:center;
            ...
            <svg width="80" height="120" viewBox="0 0 80 120"
                 style="animation:bounce_{safe_id} 1.2s ease-in-out infinite;">
                <style>
                    @keyframes bounce_{safe_id} {{
                        0%,100% {{ transform:translateY(0); }}
                        50%     {{ transform:translateY(-10px); }}
                    }}
                    @keyframes sway_{safe_id} {{
                ...
                <g style="transform-origin:40px 40px;
                           animation:sway_{safe_id} 1.2s ease-in-out infinite;">
    ...
    <style>
        *{{box-sizing:border-box;margin:0;padding:0;}}
        ...
        .card{{display:flex;gap:20px;padding:4px 2px;align-items:flex-start;}}
        .anim-col{{flex:0 0 240px;}}
        .info-col{{flex:1;min-width:0;}}
        .ex-name{{font-size:1.05rem;font-weight:600;color:#1f2937;margin-bottom:10px;}}
        .badges{{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px;}}
        .badge{{border-radius:8px;padding:3px 10px;font-size:0.8rem;
                background:#f3f4f6;color:#374151;}}
        .badge-diff{{font-weight:600;background:#ecfdf5;color:{diff_color};}}
        .target{{font-size:0.8rem;color:#6b7280;margin-bottom:12px;}}
        .inst-box{{background:#f9fafb;border-radius:12px;padding:14px;
                   border-left:4px solid #4f46e5;}}
        .inst-label{{font-size:0.82rem;font-weight:600;color:#4f46e5;margin-bottom:6px;}}
        .inst-text{{font-size:0.86rem;color:#374151;line-height:1.75;}}
    </style></head><body>
    <div class="card">
        <div class="anim-col">{animation_block}</div>
        <div class="info-col">
            <div class="ex-name">{icon} {ex_name}</div>
            <div class="badges">
            ...
            <div class="inst-box">
                <div class="inst-label">📋 How to do it</div>
                <div class="inst-text">{instructions}</div>
            </div>
        </div>
    </div>
    </body></html>"""
...
def _render_alert_settings(patient: dict, username: str):
    """Settings page — update family alert emails and test the alert system."""
    st.markdown("### ⚙️ Settings & Alert Configuration")

    # ── Current emails ────────────────────────────────────────────────────
    current_emails = patient.get("family_emails", [])
    st.markdown("#### 📧 Family Alert Emails")
    st.markdown(
        "These are the email addresses that receive alerts when blood pressure is high "
        "or recovery needs attention. Enter one address per line."
    )

    if current_emails:
        st.success(f"✅ **{len(current_emails)} address(es) currently registered:**")
        for em in current_emails:
            st.markdown(f"• `{em}`")
    else:
        st.warning("⚠️ **No alert emails registered.** Alerts cannot be sent until you add at least one address below.")

    st.markdown("---")

    # ── Update emails form ────────────────────────────────────────────────
    st.markdown("#### Update Email Addresses")
    new_emails_raw = st.text_area(
        "Enter family email addresses (one per line)",
        value="\n".join(current_emails),
        height=120,
        placeholder="daughter@gmail.com\nson@yahoo.com\ndoctor@hospital.com",
        key="settings_emails"
    )

    if st.button("💾 Save Email Addresses", key="save_emails"):
        new_emails = [e.strip().lower() for e in new_emails_raw.splitlines()
                      if e.strip() and "@" in e.strip()]
        if not new_emails:
            st.error("Please enter at least one valid email address.")
        else:
            update_patient_field(username, "family_emails", new_emails)
            # Also update the session state so sidebar refreshes immediately
            st.session_state["patient"]["family_emails"] = new_emails
            st.success(
                f"✅ Saved {len(new_emails)} email address(es): "
                + ", ".join(new_emails)
            )
            st.rerun()
    ...
    st.markdown(
        "Sends a test email to all registered addresses. "
        "Use this to confirm alerts are working before relying on them."
    )

    if st.button("📤 Send Test Alert Now", key="test_alert"):
        emails = patient.get("family_emails", [])
        if not emails:
            st.error("No email addresses registered. Add them above first.")
        else:
            ...
            sent = send_alert_email(
                ...
                family_emails=emails,
                alert_type="daily_complete",
                message=f"This is a TEST alert from the Stroke Recovery Monitor for {patient['patient_name']}.",
                details={
                    "Test Status":   "✅ Alert system is working correctly",
                    "Patient":       patient["patient_name"],
                    "Registered to": ", ".join(emails),
                    "Time":          datetime.now().strftime("%d %B %Y at %I:%M %p"),
                }
            )
            if sent:
                st.success(
                    f"✅ **Test alert sent successfully** to: {', '.join(emails)}\n\n"
                    "Check those inboxes now (including spam/junk folder). "
                    "If received, your alerts are fully working."
                )
            else:
                st.error(
                    ...
                    "2. Make sure you have exactly these two lines:\n\n"
                    "```\n"
                    "SENDER_EMAIL    = \"your-gmail@gmail.com\"\n"
                    "SENDER_PASSWORD = \"your-16-char-app-password\"\n"
                    "```\n\n"
                    "3. SENDER_PASSWORD must be a **Gmail App Password** "
    ...
    # ── Alert status diagnostic ────────────────────────────────────────────
    st.markdown("#### 🔍 Alert System Diagnostic")
    try:
        import streamlit as _st
        sender = _st.secrets.get("SENDER_EMAIL", None)
        if sender:
            st.success(f"✅ SENDER_EMAIL is configured: `{sender}`")
        else:
            st.error("❌ SENDER_EMAIL is NOT set in Streamlit Secrets")
        pw = _st.secrets.get("SENDER_PASSWORD", None)
        if pw:
            st.success("✅ SENDER_PASSWORD is configured (hidden)")
        else:
            st.error("❌ SENDER_PASSWORD is NOT set in Streamlit Secrets")
    except Exception:
        st.error("❌ Could not read Streamlit Secrets. Email alerts will not work.")

    emails_check = patient.get("family_emails", [])
    if emails_check:
        st.success(f"✅ {len(emails_check)} alert email(s) registered")
    else:
        st.error("❌ No alert emails registered — alerts have no destination")
...
def _render_family_overview(today_log, logs, patient, username, alerts, unread, days_post):
    ...
    if today_log:
        ...
        status_msg   = {
            0: f"⚠️ {name}'s recovery metrics need clinical attention today. "
               "Consider contacting their doctor.",
            1: f"{name} is in a plateau phase. Their exercises and routine may need adjustment.",
            2: f"{name} is making solid recovery progress today. Keep encouraging them!"
        }
        ...
        with col_recs:
            st.markdown(f"#### Today's Recommendations for {name}'s Care Team")
            recs = get_recommendations(pred, today_log)
            for r in recs:
                card_class = "rec-card-urgent" if r["priority"] == "urgent" else (
                             "rec-card-good"   if r["priority"] == "good"   else "")
        ...
        st.markdown("They have not checked in yet today. You can send them a reminder.")
    ...
    It does not replace clinical assessment. If you are concerned about the patient's condition, contact their medical team directly.
    </div>""", unsafe_allow_html=True)
