...
def _lottie_player(lottie_url: str, height: int = 260) -> str:
    """
    Returns an HTML string that embeds a Lottie animation using the
    official dotlottie-wc web component from LottieFiles.
    Works with both .lottie and classic .json URLs.
    No extra Python package required — uses st.components.v1.html().
    """
    return f"""
    <script
    ...
    <div style="display:flex; justify-content:center; align-items:center;
                background:#f8f7ff; border-radius:16px; padding:12px;
                margin-bottom:4px; min-height:{height}px;">
        <dotlottie-wc
            src="{lottie_url}"
            autoplay
            loop
            style="width:{height}px; height:{height}px;"
            onerror="this.style.display='none'; document.getElementById('lottie-fallback-{abs(hash(lottie_url))}').style.display='flex';">
        </dotlottie-wc>
        <div id="lottie-fallback-{abs(hash(lottie_url))}"
             style="display:none; flex-direction:column; align-items:center;
                    justify-content:center; height:{height}px; color:#9ca3af;
                    font-size:3rem;">
            🏃
            <div style="font-size:0.8rem; margin-top:8px; color:#6b7280;">
                Animation loading…
            </div>
        </div>
    </div>
...
def _render_exercise_guide():
    ...
    diff_colors = {"Beginner": "#059669", "Intermediate": "#d97706", "All levels": "#4f46e5"}

    for i, ex in enumerate(EXERCISES):
        diff_color = diff_colors.get(ex["difficulty"], "#6b7280")

        with st.expander(f"{ex['icon']} {ex['name']}  —  {ex['reps']}  ·  {ex['difficulty']}",
                         expanded=(i == 0)):

            col_anim, col_info = st.columns([1, 1.3])

            with col_anim:
                lottie_url = ex.get("lottie", "")
                if lottie_url:
                    components.html(
                        _lottie_player(lottie_url, height=260),
                        height=290,
                        scrolling=False
                    )
                else:
                    st.markdown(
                        f"<div style='font-size:5rem;text-align:center;padding:2rem;'>{ex['icon']}</div>",
                        unsafe_allow_html=True
                    )
            ...
            with col_info:
                ...
                <div style="padding: 0.5rem 0;">
                    <div style="font-size:1.1rem; font-weight:600; color:#1f2937;
                                margin-bottom:0.5rem;">{ex['icon']} {ex['name']}</div>

                    <div style="display:flex; flex-wrap:wrap; gap:8px; margin-bottom:1rem;">
                        <span style="background:#f3f4f6; border-radius:8px; padding:4px 10px;
                        ...
                        <span style="background:#ecfdf5; border-radius:8px; padding:4px 10px;
                                     font-size:0.82rem; color:{diff_color}; font-weight:600;">
                            {ex['difficulty']}
                        </span>
                    </div>
                    ...
                    <div style="background:#f9fafb; border-radius:12px; padding:1rem;
                        ...
                        <div style="font-size:0.85rem; font-weight:600; color:#4f46e5;
                                    margin-bottom:0.4rem;">📋 How to do it</div>
